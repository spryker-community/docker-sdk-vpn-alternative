...
function Paas::environments() {
    ...
    ENVIRONMENTS=$(aws ssm --region $AWS_REGION get-parameters-by-path --path / --recursive --with-decryption --output json --no-cli-pager | jq -r '.Parameters[] | select(.Name | contains("'$SEARCH_PATTERN'")) | "\(.Name)=\(.Value)"' | awk -F '/' '{print $NF}' |awk -F"=" '{ print $NF }')
    ...
    for arg in "${@}"; do
        case "${arg}" in
            'database')
                ...
                aws ssm --region $AWS_REGION get-parameters-by-path --path / --recursive --with-decryption --output json --no-cli-pager | jq -r --arg ENVIRONMENT "$ENVIRONMENT/" --arg SEARCH_PATTERN "$SEARCH_PATTERN" '.Parameters[] | select(.Name | contains($ENVIRONMENT) and contains($SEARCH_PATTERN)) | "\(.Name)=\(.Value)"' | awk -F '/' '{print $NF}'
                ;;
            'database-ro-replica')
...
function Paas::create-tunnel() {
    ...
    for SERVICE_CONNECTION_PARAMTER in $(aws ssm get-parameters --region ${AWS_REGION} --names ${SEARCH_STRING_RESULT}  --with-decryption  | jq -r '.Parameters[] | "\(.Name)=\(.Value)"' | awk -F '/' '{print $NF}'); do 
